...
func RewriteFile(content string, refs []ComposeImageRef, digests map[int]string) string {
	...
	// prefix of another, so each is replaced at the column it was written at rather
	// than at the first place its text occurs. Taking the rightmost first keeps the
	// columns of the images still to come valid, since inserting a digest only
...
func rightmostFirst(refs []ComposeImageRef) []int {
