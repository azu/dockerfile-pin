...
func RewriteFile(content string, refs []GitLabImageRef, digests map[int]string) string {
	...
	// A flow sequence writes several references on one line, and one of them may be
	// a prefix of another, so each is replaced at the column it was written at rather
	// than at the first place its text occurs. Taking the rightmost first keeps the
	// columns of the references still to come valid, since inserting a digest only
	// shifts what follows it.
	for _, i := range rightmostFirst(refs) {
		...
		lines[lineIdx] = replaceAt(lines[lineIdx], ref.Column, ref.RawRef, ref.ImageRef+"@"+digest)
	}
...
func rightmostFirst(refs []GitLabImageRef) []int {
